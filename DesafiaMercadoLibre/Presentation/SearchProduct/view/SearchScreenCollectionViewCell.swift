...
class SearchScreenCollectionViewCell: UICollectionViewCell {
    ...
    func configure(model: ProductView) {
        setupView()
        setupConstraints()
        setup(with: model)
    }
    ...
    func setup(with model: ProductView) {
        titleLabel.text = model.title
        amountLabel.text = model.price
        descriptionLabel.text = model.condition
        imageView.sd_setImage(with: URL(string: model.thumbnail.replacingOccurrences(of: "http", with: "https")), completed: nil)
    }
